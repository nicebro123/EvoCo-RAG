...
| ECR-8 | 跨数据集和强基线验证 | 方法是否只是 PopQA 适配，还是通用 RAG 机制 | **部分完成，待跑数成表**：dataset pack 和 `multidataset_fast_2gpu.yaml` 已覆盖 PopQAStandard、HotpotQA、NQ、ASQA、PopQA retrieval | PopQA + NQ + HotpotQA + ASQA 的 full runs、多强基线和完整消融 | `data.py`、`configs/`、`scripts/run_ablations.py`、`evaluation/metrics.py` |
